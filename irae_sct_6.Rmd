...
run_scDblFinder_on_seurat <- function(seurat_obj,
  ...
  if (verbose) {
    message("Running scDblFinder (per-sample = ", !is.null(sample_col), ") …")
  }
...
validate_doublet_removal_scDblFinder <- function(seurat_obj,
  ...
  if (length(feat_meta) > 0) {
    v1 <- try(VlnPlot(seurat_obj, features = feat_meta,
                      group.by = class_col, pt.size = 0.1, combine = TRUE), silent = TRUE)
...
```
```{r}
# Validation summaries and QC plots
validate_doublet_removal_scDblFinder(merged_Seurat, class_col = "scDblFinder.class", sample_col = "orig.ident")
