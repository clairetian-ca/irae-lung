...
validate_doublet_removal_scDblFinder <- function(seurat_obj,
  ...
  if (length(feat_meta) > 0) {
    v1 <- try(VlnPlot(seurat_obj, features = feat_meta,
                      group.by = class_col, pt.size = 0, combine = TRUE), silent = TRUE)
...
try(utils::write.csv(label_df, file = "scDblFinder_cell_labels.csv", row.names = FALSE), silent = TRUE)
  ...
  # 1) Median QC metrics by class (singlet/doublet)
  qc_cols <- intersect(c("nFeature_RNA", "nCount_RNA", "percent.mt"), colnames(merged_Seurat@meta.data))
  if (length(qc_cols) > 0) {
    med_summ <- sapply(qc_cols, function(f) tapply(merged_Seurat@meta.data[[f]],
                                                   merged_Seurat$scDblFinder.class,
                                                   median, na.rm = TRUE))
    med_summ <- as.data.frame(med_summ)
    med_summ$class <- rownames(med_summ)
    med_summ <- med_summ[, c("class", qc_cols), drop = FALSE]
    print(med_summ)
  }

  # 2) Per-sample doublet rate (percent)
  if (!is.null(merged_Seurat$orig.ident)) {
    per_sample_rate <- tapply(merged_Seurat$scDblFinder.class == "doublet",
                              merged_Seurat$orig.ident,
                              function(x) mean(x, na.rm = TRUE))
    per_sample_rate <- sort(per_sample_rate, decreasing = TRUE)
    cat("Per-sample doublet rates (%):\n")
    print(round(100 * per_sample_rate, 2))
  }

  # 3) Score separation and violin plot for scDblFinder.score
  if ("scDblFinder.score" %in% colnames(merged_Seurat@meta.data)) {
    cat("scDblFinder.score summary by class:\n")
    print(tapply(merged_Seurat$scDblFinder.score,
                 merged_Seurat$scDblFinder.class, summary))
    try(print(VlnPlot(merged_Seurat, features = "scDblFinder.score",
                      group.by = "scDblFinder.class", pt.size = 0)), silent = TRUE)
  }
...
```
```{r step5, eval = getOption("start_step", 1) < 6}
